...
test('mobile framing keeps playable rails and goals clear of HUD reservations', {
  ...
  const { THREE } = await import('./helpers/real-three-session-fixture.mjs');
  const { hudReservePixels, playerCameraPose, pitchFramePoints, tacticalViewBounds } = await import('../src/fx/player-camera-view-poses.js');
  ...
  for (const [width, height] of [[320,844],[375,844],[390,844],[430,844],[568,320],[844,390],[768,1024],[1024,768]]) {
    ...
    for (const mode of ['free', 'tactical']) for (const type of ['ruler-seesaw', 'departing-lorry']) {
      ...
      const bounds = mode === 'tactical' ? tacticalViewBounds() : null;
      const reserve = hudReservePixels(width, height);
      for (const point of pitchFramePoints(session)) {
        const p = point.project(camera);
        assert.ok(Math.abs(p.x) < (bounds?.x ?? .95), `${width} ${mode} horizontal`);
        if (bounds) {
          assert.ok(p.y < bounds.top && p.y > bounds.bottom, `${width} ${mode} tactical frame`);
          continue;
        }
        const { top, bottom } = reserve;
