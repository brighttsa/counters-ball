...
test('mobile framing keeps playable rails and goals clear of HUD reservations', {
  ...
  for (const [width, height] of [[320,844],[375,844],[390,844],[430,844],[568,320],[844,390],[768,1024],[1024,768]]) {
    ...
    for (const mode of ['broadcast', 'tactical']) for (const type of ['ruler-seesaw', 'departing-lorry']) {
      ...
      const pose = playerCameraPose(camera, mode, { level: { mechanic: { type } } });
      ...
      for (const x of [-end,end]) for (const z of [-1.24,1.24]) for (const elevation of [0,.28]) {
        const p = new THREE.Vector3(x, elevation, z).project(camera);
        assert.ok(Math.abs(p.x) < .9, `${width} ${mode} horizontal`);
