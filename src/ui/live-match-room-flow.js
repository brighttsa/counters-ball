...
export function createLiveMatchRoomFlow({ level, baseUrl, showTitle, onStart }) {
  ...
  return {
    ...
    async create() { status('Creating your room…'); try { const result = await createLiveRoom(api, { levelId: level.id, homeName: $('live-room-name').value }); open(result); status(`Room created. Your code is ${result.id}. Send the invite.`); } catch { status('Could not create a room. Check your connection.'); } },
