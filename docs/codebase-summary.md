...
| `SoundtrackDirector` (`audio/soundtrack-music-director.js`) | The only music player: `request(id, {dip})` is idempotent; crossfades tracks; decodes at 32 kHz, keeps two; goal duck. Screens map to tracks in `soundtrack-track-list-and-screen-routing.js`; `soundtrack-app-wiring.js` hooks `MenuScreens.onShow`; preferences in `music-and-effects-audio-settings.js` |
